...
def _fake_child_run(config, workspace_root, **kwargs):
    ...
    child_index = mask_seed % 100
    batch_mean = [1.0, 1.2, 0.8][child_index]
    stream_total = [10.0, 12.0, 8.0][child_index]
    return {
        "PU-Acc": float(mask_seed % 100),
        "FO-Acc": float(mask_seed % 100 + 10),
        "runtime": 1.25,
        "online_batch_runtime_mean_sec": batch_mean,
        "online_batch_runtime_std_sec": 0.1,
        "online_batch_runtime_median_sec": batch_mean,
        "online_batch_runtime_p95_sec": batch_mean,
        "online_compute_runtime_sec": stream_total,
        "adapt_batch_runtime_mean_sec": batch_mean / 2,
        "adapt_batch_runtime_std_sec": 0.05,
        "adapt_runtime_total_sec": stream_total / 2,
        "pu_batch_runtime_mean_sec": batch_mean / 2,
        "pu_batch_runtime_std_sec": 0.05,
        "pu_runtime_total_sec": stream_total / 2,
        "gpu_peak_allocated_mean_mb": [100.0, 120.0, 80.0][child_index],
        "gpu_peak_allocated_max_mb": [110.0, 130.0, 90.0][child_index],
        "gpu_peak_reserved_mean_mb": [150.0, 170.0, 130.0][child_index],
        "gpu_peak_reserved_max_mb": [160.0, 180.0, 140.0][child_index],
...
def main():
    with tempfile.TemporaryDirectory(prefix="iclr2027_random_summary_") as temp_dir:
        config = load_yaml(osp.join(PROJECT_DIR, "configs", "otta_fc_lbi_protocol_20260817_v1.yaml"))
        config.update(
            {
        ...
        assert summary["mask_seeds"] == [202600, 202601, 202602]
        assert summary["requested_budget"] == 0.001
        assert summary["selection_seed"] == 2026
        ...
        assert summary["online_batch_runtime_mean_sec"] == 1.0
        assert summary["online_compute_runtime_sec"] == 10.0
        assert summary["random_total_online_compute_runtime_sec"] == 30.0
        assert summary["gpu_peak_allocated_max_mb"] == 130.0
        for field in (
            "selection",
            "mask_static",
            "candidate_scope",
        ...
        assert persisted["mask_seeds"] == [202600, 202601, 202602]
