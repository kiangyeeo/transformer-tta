...
def resolve_target_order(config, target_size):
    """Build the frozen outer-stream order used by every OTTA method."""
    seed = int(config["seed"])
    if config["data"]["dataset"] == "VISDA-C":
        from visda_otta.adapter import build_target_order

        return build_target_order(seed, target_size)

    sampler_generator = torch.Generator()
    sampler_generator.manual_seed(seed)
    order = torch.randperm(
        target_size, generator=sampler_generator
    ).tolist()
    return order, {
...
def build_order_record(
    ...
    """Describe the fixed sample identities and worker RNG provenance."""
    return {
        "target": {
            **target_order_record,
            "indices": list(target_order),
            "sha256": _order_digest(target_order),
        },
        "test": {
            "sampler": "sequential",
            "indices": list(test_order),
            "sha256": _order_digest(test_order),
        },
        "target_worker_seed_base": int(seed) + 1,
        "test_worker_seed_base": int(seed) + 2,
    }
