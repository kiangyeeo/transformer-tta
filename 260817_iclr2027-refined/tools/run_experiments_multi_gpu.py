...
def checkpoint_eligible(experiment):
    """LBI variants use the protocol's completed-batch stream checkpoint."""
    return experiment.get('variant') in {
        'module_lbi', 'conv_out_lbi', 'conv_filter_lbi',
    }
...
def command_for_experiment(experiment, gpu, resume_run_dir=None, enable_stream_checkpoint=False, workers_per_gpu=None):
    ...
    if resume_run_dir and not eligible:
        raise ValueError(
            'stream checkpoint/resume is only allowed for LBI variants'
        )
    ...
    if workers_per_gpu is not None:
        ...
        # Diagnostic plans may require one process per GPU without claiming
        # formal-runtime comparability.  Preserve the old default for plans
        # that predate this field, while allowing an explicit false value.
        if experiment.get('runtime_comparable', workers_per_gpu == 1):
...
def validate_visda_batch_size(plan):
    """Check the frozen VISDA-C batch size across canonical and shard plans."""
    entries = [
        experiment
        for experiment in plan.get('experiments', [])
    ...
    for experiment in entries:
        values = []
        scientific = experiment.get('scientific_config')
        if scientific is not None:
            try:
                values.append(int(scientific['data']['batch_size']))
            except (KeyError, TypeError, ValueError) as error:
                raise RuntimeError(
                    'VISDA-C scientific_config must contain data.batch_size '
                    f"for {experiment.get('experiment_key')!r}"
                ) from error
        full_stream = experiment.get('full_stream')
        if full_stream is not None:
            try:
                values.append(int(full_stream['batch_size']))
            except (KeyError, TypeError, ValueError) as error:
                raise RuntimeError(
                    'VISDA-C full_stream must contain batch_size '
                    f"for {experiment.get('experiment_key')!r}"
                ) from error
        if not values:
            raise RuntimeError(
                'VISDA-C plan entry must declare batch_size in scientific_config '
                f"or full_stream for {experiment.get('experiment_key')!r}"
            )
        if len(set(values)) != 1:
            raise RuntimeError(
                'VISDA-C plan has conflicting scientific_config/full_stream '
                f"batch sizes for {experiment.get('experiment_key')!r}: {values}"
            )
