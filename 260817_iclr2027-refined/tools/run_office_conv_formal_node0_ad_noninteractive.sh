...
PLAN_DIR="${FORMAL_ROOT}/plans/node0_ad"
PLAN_PATH="${PLAN_DIR}/plan.json"
MATRIX_PATH="${PLAN_DIR}/matrix.yaml"
PREPARE_RECORD="${FORMAL_ROOT}/phase_records/node0_ad/PREPARE.md"
RUNS_ROOT="${FORMAL_ROOT}/runs"
LOGS_ROOT="${FORMAL_ROOT}/launcher_logs/node0_ad"
COMMAND_HISTORY="${FORMAL_ROOT}/command_history/node0_ad_commands.sh"
LOCK_PATH="${FORMAL_ROOT}/locks/node0_ad.lock"

mkdir -p "$(dirname "${LOCK_PATH}")"
if [[ ! -f "${PLAN_PATH}" || ! -f "${MATRIX_PATH}" || ! -f "${PREPARE_RECORD}" ]]; then
  echo "ERROR: Node 0 formal plan, matrix, or PREPARE record is missing." >&2
  exit 1
fi
if ! mkdir "${LOCK_PATH}" 2>/dev/null; then
  echo "ERROR: Node 0 launcher lock already exists: ${LOCK_PATH}" >&2
  exit 1
fi
cleanup() {
  local status=$?
...
conda run --no-capture-output -n SHOT_TTA python \
  "${PROJECT_ROOT}/tools/prepare_office_conv_formal_node0_ad.py" \
  --matrix "${MATRIX_PATH}" \
