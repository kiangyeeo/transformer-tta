#!/usr/bin/env python3
"""Build and statically validate Node 5's frozen Office W->D Conv plan."""
...
NODE = "node5_wd"
TRANSFER = {"source": 2, "target": 1, "identifier": "WD"}
...
def assert_frozen_inputs(matrix, base_config):
    ...
    for filename, expected_heading in required_protocols:
    ...
    for field, value in expected_matrix.items():
...
def assert_matrix(matrix):
    ...
    if matrix.get("dataset") != "office" or matrix.get("transfer") != TRANSFER:
        fail("Node 5 must be exactly Office Webcam->DSLR: source=2, target=1")
    ...
    if "0.005" in serialized or "_lbi" in serialized:
    ...
    if observed != EXPECTED_CONDITIONS or len(conditions or []) != len(EXPECTED_CONDITIONS):
        fail("matrix condition families do not match Node 5's required seven families")
    ...
    for mode, count in GROUP_COUNTS.items():
        budget = matrix.get("expected_group_budgets", {}).get(mode, {})
        if budget.get("total_group_count") != count:
            fail(f"{mode} total group count drifted")
        if budget.get("K_G") != [EXPECTED_K[mode][rho] for rho in RHOS]:
...
def build_entry(base_config, matrix, variant, group_mode, rho):
    ...
    config["data"].update({"dataset": "office", "source": 2, "target": 1})
    ...
    command = [
        ...
        "--dataset", "office", "--source", "2", "--target", "1", "--seed", "2026",
    ...
    return {
        ...
        "effective_overrides": {
            "dataset": "office", "source": 2, "target": 1, "seed": FORMAL_SEED,
...
def validate_plan(plan, runs_root):
    ...
    if {(entry["variant"], entry["requested_budget"]) for entry in entries} != expected:
        fail("plan conditions are not exactly the required 19 entries")
    for entry in entries:
        if (entry["dataset"], entry["source"], entry["target"], entry["seed"]) != ("office", 2, 1, 2026):
...
def main():
    ...
    print(json.dumps({"valid": True, "experiment_count": 19, "transfer": "WD"}))
