...
PLAN_PATH="${CONTROL_ROOT}/plans/node6_omega_0p1__lr_0p005.jsonl"
PREPARE_RECORD="${CONTROL_ROOT}/node_records/node6_PREPARE.md"
PREPARE_TOOL="${PROJECT_ROOT}/tools/prepare_conv_lbi_r2_out_node6.py"
RUNS_ROOT="${PROJECT_ROOT}/experiment_logs/office_conv_lbi_r2_out_joint_sweep_seed2026_20260828/runs/${CELL_ID}"
LOGS_ROOT="${CONTROL_ROOT}/launcher_logs/node6_${CELL_ID}"
COMMAND_HISTORY="${CONTROL_ROOT}/command_history/node6_${CELL_ID}_commands.sh"
LOCK_PATH="${CONTROL_ROOT}/locks/node6_${CELL_ID}.lock"

if [[ ! -f "${PLAN_PATH}" || ! -f "${PREPARE_RECORD}" || ! -f "${PREPARE_TOOL}" ]]; then
  echo "ERROR: Node 6 R2 plan, PREPARE record, or verifier is missing." >&2
  exit 1
fi

mkdir -p "$(dirname "${LOCK_PATH}")"
if ! mkdir "${LOCK_PATH}" 2>/dev/null; then
  echo "ERROR: Node 6 R2 launcher lock already exists: ${LOCK_PATH}" >&2
  exit 1
fi
