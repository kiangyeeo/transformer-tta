...
PLAN_PATH="${BURST_ROOT}/plans/node1_filter_r0f.json"
PLAN_JSONL="${BURST_ROOT}/plans/node1_filter_r0f.jsonl"
PREPARE_TOOL="${BURST_ROOT}/plans/prepare_node1_filter_r0f.py"
PREPARE_RECORD="${BURST_ROOT}/node_records/node1_filter_r0f_PREPARE.md"
RUNS_ROOT="${BRANCH_ROOT}/runs"
LOGS_ROOT="${BURST_ROOT}/launcher_logs/node1_filter_r0f"
COMMAND_HISTORY="${BURST_ROOT}/command_history/node1_filter_r0f_commands.sh"
LOCK_PATH="${BURST_ROOT}/locks/node1_filter_r0f.lock"

if [[ ! -f "${PLAN_PATH}" || ! -f "${PLAN_JSONL}" || ! -f "${PREPARE_TOOL}" || ! -f "${PREPARE_RECORD}" ]]; then
  echo "ERROR: Node 1 plan or PREPARE artifacts are missing; rerun MODE=PREPARE." >&2
  exit 1
fi
