...
PLAN_DIR="${FORMAL_ROOT}/plans/node5_wd"
PLAN_PATH="${PLAN_DIR}/plan.json"
MATRIX_PATH="${PLAN_DIR}/matrix.yaml"
PREPARE_RECORD="${FORMAL_ROOT}/phase_records/node5_wd/PREPARE.md"
RUNS_ROOT="${FORMAL_ROOT}/runs"
LOGS_ROOT="${FORMAL_ROOT}/launcher_logs/node5_wd"
COMMAND_HISTORY="${FORMAL_ROOT}/command_history/node5_wd_commands.sh"
LOCK_PATH="${FORMAL_ROOT}/locks/node5_wd.lock"

mkdir -p "$(dirname "${LOCK_PATH}")"
if [[ ! -f "${PLAN_PATH}" || ! -f "${MATRIX_PATH}" || ! -f "${PREPARE_RECORD}" ]]; then
  echo "ERROR: Node 5 formal plan, matrix, or PREPARE record is missing." >&2
  exit 1
fi
...
conda run --no-capture-output -n SHOT_TTA python \
  "${PROJECT_ROOT}/tools/prepare_office_conv_formal_node5_wd.py" \
  --matrix "${MATRIX_PATH}" \
