# Office LBI Final Formal Evaluation — Machine 3 — Budget 0.002 + Global Aggregator
...
Verify that budget `0.002` is frozen exactly as:
...
```text
experiment_logs/shot_otta_office_lbi_final_formal_20260818/matrices/machine3/
```
...
```text
experiment_logs/shot_otta_office_lbi_final_formal_20260818/plans/machine3/plan.json
```
...
```text
tools/run_office_lbi_final_formal_budget_002_noninteractive.sh
```
...
bash tools/run_office_lbi_final_formal_budget_002_noninteractive.sh
```
...
```text
experiment_logs/shot_otta_office_lbi_final_formal_20260818/locks/machine3.lock
```
...
```text
bash -n tools/run_office_lbi_final_formal_budget_002_noninteractive.sh
git diff --check
...
```text
experiment_logs/shot_otta_office_lbi_final_formal_20260818/phase_records/machine3/PREPARE_READY.json
```
...
```text
experiment_logs/shot_otta_office_lbi_final_formal_20260818/phase_records/machine3/PREPARE.md
```
...
```text
experiment_logs/shot_otta_office_lbi_final_formal_20260818/reports/machine3/
```
...
```text
phase_records/machine3/FINALIZE.md
phase_records/machine3/FINALIZE_READY.json
```

Only create `FINALIZE_READY.json` when all 6 runs are complete and summarized.

## 7. Global Office final-result aggregation — Machine 3 aggregator

After Machine 3 local FINALIZE, check:
...
If any marker is absent, report which one is pending and stop. Do not launch anything.

When all three are present, combine exactly:

```text
budget .0005: 6 final LBI transfers
budget .001 : 6 final LBI transfers
budget .002 : 6 final LBI transfers
```

Require:

```text
18 / 18 final formal LBI runs complete
```

Use the frozen tuples from:
...
Create the final Office paper-oriented outputs:

```text
reports/global/OFFICE_FINAL_LBI_18_RUNS.csv
reports/global/OFFICE_FINAL_RESULTS_TABLE.csv
reports/global/OFFICE_FINAL_RESULTS_TABLE.md
reports/global/OFFICE_FINAL_EFFICIENCY_TABLE.csv
reports/global/OFFICE_FINAL_EFFICIENCY_TABLE.md
reports/global/OFFICE_FINAL_ANALYSIS.md
reports/global/OFFICE_FINAL_ANALYSIS.json
```

The final result table should include:
...
Module dense
Random @ .0005/.001/.002
Magnitude @ .0005/.001/.002
Saliency @ .0005/.001/.002
LBI @ .0005/.001/.002
```

Report PU and FO.

For each LBI budget report:
...
For efficiency report at least:

```text
online batch runtime
online compute runtime
FO evaluation runtime
peak allocated GPU memory
```

Only make strict runtime comparisons where hardware provenance is comparable.

Highlight the frozen budget with highest final formal mean FO, but do not retune anything if formal rerun ranking differs slightly from the tuning sweep.

Explicitly state:

```text
Office LBI hyperparameter tuning is closed.
These 18 runs are final formal evaluation runs.
No further Office LBI tuning is recommended.
```
...
```text
FINALIZE complete for machine3 / budget 0.002.
