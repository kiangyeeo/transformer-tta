# Office LBI Final Joint Sweep — Machine 4 + Global Aggregator
...
```text
experiment_logs/shot_otta_office_lbi_joint_sweep_20260818/matrices/machine4/
```
...
|---:|---:|---|---:|---:|---:|
| 0.0005 | 262 | A4 | 0.10 | 1.5 | 0.50 |
| 0.001 | 524 | A8 | 0.15 | 1.0 | 1.00 |
...
```text
experiment_logs/shot_otta_office_lbi_joint_sweep_20260818/plans/machine4/plan.json
```
...
```text
tools/run_office_lbi_joint_sweep_machine4_noninteractive.sh
```
...
bash tools/run_office_lbi_joint_sweep_machine4_noninteractive.sh
```
...
```text
experiment_logs/shot_otta_office_lbi_joint_sweep_20260818/locks/machine4.lock
```
...
```text
bash -n tools/run_office_lbi_joint_sweep_machine4_noninteractive.sh
```
...
```text
experiment_logs/shot_otta_office_lbi_joint_sweep_20260818/phase_records/machine4/PREPARE_READY.json
```
...
```text
experiment_logs/shot_otta_office_lbi_joint_sweep_20260818/phase_records/machine4/PREPARE.md
```
...
```text
experiment_logs/shot_otta_office_lbi_joint_sweep_20260818/reports/machine4/
```
...
```text
phase_records/machine4/FINALIZE.md
phase_records/machine4/FINALIZE_READY.json
```

Only create `FINALIZE_READY.json` when the local 132-run grid plus 12 references is complete and validly summarized.

## 7. Global Office selection — Machine 4 aggregator

After Machine 4 local FINALIZE, check:

```text
phase_records/machine1/FINALIZE_READY.json
phase_records/machine2/FINALIZE_READY.json
phase_records/machine3/FINALIZE_READY.json
phase_records/machine4/FINALIZE_READY.json
```

If any is absent, report which one is pending and stop. Do not launch anything.

When all four exist, consolidate exactly these 8 anchor/budget pairs:

```text
budget .0005: A6, A1, A4
budget .001 : A6, A4, A8
budget .002 : A2, A3
```
...
```text
12 omega×stage2_lr configurations
6 transfers each
```
...
```text
576 run rows
= 528 new joint-sweep runs
+ 48 reused Stage-1 reference runs
```

Refuse global selection if counts or identities do not match.

For each budget independently, eligible configurations require:

```text
6/6 scientific-valid
6/6 pass the 90% utilization hard gate
```

Then rank:

```text
1. mean_FO_margin descending
2. worst_transfer_FO_margin descending
3. mean_FO descending
4. 95%-utilization robustness diagnostics
5. stage1_steps_max ascending
6. stage1_steps_mean ascending
```

PU is report-only.

Freeze exactly one tuple per budget:
...
```text
selected_configs/office_budget_0005_final_lbi.json
selected_configs/office_budget_001_final_lbi.json
selected_configs/office_budget_002_final_lbi.json
selected_configs/OFFICE_FINAL_LBI_TUPLES.json
```

Also identify:

```text
overall_best_budget_by_mean_FO
```

for paper highlighting, but keep all three budget winners.

Create:

```text
reports/global/OFFICE_LBI_JOINT_SWEEP_ALL_576.csv
reports/global/OFFICE_LBI_JOINT_SWEEP_CONFIGS.csv
reports/global/OFFICE_LBI_FINAL_SELECTION.md
reports/global/OFFICE_LBI_FINAL_SELECTION.json
```

Explicitly state that Office LBI hyperparameter search is closed.

Do not expand the grid.

Recommend, but do not launch, the final formal LBI rerun:

```text
3 budgets × 6 transfers = 18 runs
1 experiment/GPU
runtime_comparable=true
```

Those 18 are frozen-parameter evaluation runs, not tuning.


Finish with:

```text
FINALIZE complete for machine4.
