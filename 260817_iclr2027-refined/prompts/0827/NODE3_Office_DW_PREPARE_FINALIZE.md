...
## MODE=PREPARE
1. Verify frozen protocol/revision and the exact D→W transfer mapping; otherwise stop.
2. Create matrix + plan containing exactly the 19 conditions above.
3. Verify budgets are only `.0005/.001/.002`; no `.005`, no LBI.
4. Create `tools/run_office_conv_formal_node3_dw_noninteractive.sh`.
5. Static checks only: condition count, plan/identity/output uniqueness, Random 3-mask semantics, `bash -n`, `git diff --check`.
6. Write `experiment_logs/office_conv_baselines_seed2026_formal_20260827/phase_records/node3_dw/PREPARE.md`.
...
bash tools/run_office_conv_formal_node3_dw_noninteractive.sh
```

## MODE=FINALIZE
1. Aggregate only this node's D→W formal runs.
...
5. Write `experiment_logs/office_conv_baselines_seed2026_formal_20260827/phase_records/node3_dw/FINALIZE.md`.
