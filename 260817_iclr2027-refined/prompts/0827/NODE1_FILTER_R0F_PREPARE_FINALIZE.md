...
|---:|---|---:|---:|---:|---:|
| 1 | F4 | 0.2 | 0.25 | .0005 | 3276 |
| 2 | F5 | 0.3 | 0.25 | .0005 | 3276 |
| 3 | F6 | 0.4 | 0.25 | .0005 | 3276 |
| 4 | F7 | 0.3 | 0.125 | .0005 | 3276 |
...
```text
plan:
experiment_logs/conv_lbi_32gpu_burst_20260827/plans/node1_filter_r0f.jsonl

launcher:
tools/run_conv_lbi_32gpu_node1_filter_r0f.sh

prepare record:
experiment_logs/conv_lbi_32gpu_burst_20260827/node_records/node1_filter_r0f_PREPARE.md

finalize record:
experiment_logs/conv_lbi_32gpu_burst_20260827/node_records/node1_filter_r0f_FINALIZE.md
```
...
9. Run static checks only:
   ...
   - uniqueness checks;
   - `bash -n tools/run_conv_lbi_32gpu_node1_filter_r0f.sh`;
   - `git diff --check`.
...
12. Print only:
   ...
   bash tools/run_conv_lbi_32gpu_node1_filter_r0f.sh
   ```
13. Stop.
...
2. Verify:
   ...
   - no duplicate output;
   - finite metrics;
   - selected_group_count never exceeds `K_G=3276`;
...
This node is the **budget-level FINALIZE owner** for `rho_G=.0005`.

After auditing this node, also inspect the completed artifacts from Node 0:

```text
experiment_logs/conv_lbi_32gpu_burst_20260827/node_records/node0_filter_r0f_FINALIZE.md
```

Combine the 4 rows from Node 0 with this node's 4 rows, giving exactly 8 anchors F0-F7 for budget `.0005`.

If Node 0 is not complete yet:
- do not launch or retry anything;
- write `WAITING_FOR_NODE_0: YES`;
...
```text
experiment_logs/office_conv_lbi_r0f_filter_reachability_seed2026_20260827/
  selected_configs/FILTER_R0F_p0005_PRIMARY_BACKUP.json
  phase_records/R0F/FILTER_R0F_p0005_FINALIZE.md
```
...
End with:
```text
NODE1_FINALIZE_COMPLETE: YES/NO
PLANNED: 4
COMPLETED: X
FAILED: X
MISSING: X
