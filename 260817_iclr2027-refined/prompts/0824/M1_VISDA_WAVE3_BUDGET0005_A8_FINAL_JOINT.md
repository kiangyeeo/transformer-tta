...
# M1 — budget=.0005, anchor A8

Fixed:
```text
budget=.0005, K=262
alpha=.125, nu=.5
omega in {.003125,.00625,.0125}
lr2   in {.0025,.005,.010}
```

Existing reference; REUSE, do not rerun:
```text
omega=.00625, lr2=.005
```

Create exactly 8 NEW runs:
```text
.003125/.0025
.003125/.005
.003125/.010
...
## MODE=PREPARE
1. Verify the reference identity/config exists and matches exactly; otherwise stop.
2. Create matrix + plan with exactly the 8 NEW runs.
3. Create `tools/run_visda_wave3_m1_budget0005_A8_final_joint_noninteractive.sh`.
4. Static checks only: plan uniqueness, `bash -n`, `git diff --check`.
5. Write `${SEARCH_ROOT}/phase_records/wave3_m1/PREPARE.md`.
6. Print only the final two-line submit command, then stop.

## MODE=FINALIZE
Aggregate the full 3x3 grid = 8 new + 1 reference.
Report validity/utilization/S1/FO/PU metrics and rank with the common rule.
Write `${SEARCH_ROOT}/phase_records/wave3_m1/FINALIZE.md`.
Do NOT freeze budget=.0005 alone here and do NOT launch extra runs.
