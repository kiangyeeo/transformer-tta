...
# NODE-SPECIFIC ASSIGNMENT — node3

Node root:

```text
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/control/node3
```

Node run root:

```text
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/runs/node3
```
...
```text
<NODE_ROOT> = /inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/control/node3
<RUNS_ROOT> = /inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/runs/node3
```
...
## 9. GLOBAL_FINALIZE mode — node3 only

Only run this section **after all four node launchers have completed** and the user explicitly invokes `MODE=GLOBAL_FINALIZE`.

This mode is artifact-only:
...
Aggregate exactly the global 18-condition universe from:

```text
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/runs/node0
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/runs/node1
...
```text
18/18 completed
6 transfers × 3 budgets
0 missing
0 duplicate scientific identity
0 hash mismatch
0 invalid summary
```
...
```text
GLOBAL_FORMAL_COMPLETE = NO
```

and report the exact rows. Do not rerun them.
...
```text
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/FINALIZE.md
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/SCOPE_CLOSURE.md

/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/reports/OFFICE_OUT_LBI_FORMAL_ALL_RUNS.csv
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/reports/OFFICE_OUT_LBI_FORMAL_MACRO.csv
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/reports/OFFICE_OUT_LBI_VS_BASELINES.csv
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/reports/OFFICE_OUT_LBI_EFFICIENCY.csv
/inspire/hdd/global_user/gaoyachen-253308310317/PJ/Split-LBI/260817_iclr2027-refined/experiment_logs/office_conv_lbi_final_formal_out_channel_seed2026_20260829/reports/OFFICE_OUT_LBI_GROUP_SCALAR_SPARSITY.csv
```

`SCOPE_CLOSURE.md` must explicitly record:

```text
Original v1 final-formal design: out_channel + filter_connection
Filter formal rescue status: BLOCKED_V2
Current ICLR mainline final formal: out_channel only
Office conditions in this formal root: 18
No Filter-LBI tuple was fabricated.
```

### Accuracy reporting

For each budget report six-transfer PU and FO plus equal-weight macro.

Compare fresh formal LBI at the **same budget** against:

1. matched out-channel Random / Magnitude / Saliency;
2. `conv_module_dense`;
3. best fixed sparse method among all six conventional sparse baselines:
   out/filter × Random/Magnitude/Saliency, using that same budget;
4. same-budget per-transfer sparse oracle:
   for each transfer take the best of those same six conventional sparse baselines, then average six transfers.

Do not use cross-budget sparse oracle.

Do not retune from these comparisons.

Also compare fresh formal LBI against the frozen R2 tuning FO only as a reproducibility/consistency audit. It is not a selection step.

### Group vs scalar sparsity

For every budget report separately:

```text
rho_G
K_G
nominal group ratio
per-batch selected group count/utilization: min / mean / p05
fraction of batches below 95%
selected scalar count statistics
realized scalar ratio statistics
```

Do not call `rho_G` a scalar parameter ratio.

Do not claim physical pruning or inference FLOP reduction.

### Efficiency

Using only fresh formal runs, report:

```text
online runtime mean / std / median / p95
total online compute
Stage-1 runtime mean/share
Stage-2 runtime mean/share
PU runtime
FO evaluation runtime
peak allocated GPU memory
peak reserved GPU memory
```

Compare runtime/memory only with formal baseline artifacts where `runtime_comparable=true`.
...
No wall-clock efficiency claim unless the data supports it.

### Final status flags

End `FINALIZE.md` with:

```text
GLOBAL_FORMAL_COMPLETE: YES/NO
OFFICE_OUT_TUNING_CLOSED: YES
FILTER_STATUS: BLOCKED_V2
FINAL_FORMAL_SCOPE: OUT_CHANNEL_ONLY
RETUNING_ALLOWED_FROM_FORMAL_RESULTS: NO
```
