...
class SplitLBIEngine:
    ...
    def _validate(candidate_parameters, config):
        ...
        if config.get("group_mode") is not None:
            validate_group_mode(config["group_mode"])
    ...
    def run_step(
        ...
        """Run Stage 1, Stage 2, and Stage 3 on one online objective.

        gradient_accumulator is an optional objective-agnostic extension for
        objectives that must be evaluated in chunks. It accumulates the exact
        full-objective gradient into candidate grad fields and returns the
        same loss/parts shape as loss_closure. The legacy SHOT path leaves it
        as None and executes the original autograd path.
        """
        ...
        max_support = max_support_count(requested_budget, budget_pool_count)
        ...
        # The strict integer budget is the operative support constraint.
        # If floor(rho * pool_size) == 0, there is no legal active support,
        # so skip Stage 1 instead of running until the first inevitable
        # overshoot.  Formal FC budgets are unaffected because K > 0 there.
        stage1_branch_enabled = max_support > 0
        ...
        # FC retains its existing clone-based rollback path.  Conv uses a
        # non-destructive candidate-next-state path below, so the currently
        # committed state is itself the rollback target and no 12.8M-scalar
        # feasible-state clone is made at every valid iteration.
        feasible_state = state.clone() if group_mode is None else state
        feasible_step = 0
            ...
            for step_index in stage1_iterations:
                ...
                try:
                    if gradient_accumulator is None:
                        ...
                        for _, parameter in candidate_parameters:
                            parameter.grad = None
                        stage1_loss, _ = _loss_and_parts(
                            gradient_accumulator(candidate_parameters)
                        )
                        gradients = []
                        for name, parameter in candidate_parameters:
                            if parameter.grad is None:
                                raise RuntimeError(
                                    "gradient accumulator did not populate "
                                    f"candidate gradient: {name}"
                                )
                ...
                if group_mode is None:
                    # Preserve the frozen FC element-wise implementation.
                    ...
                    for name, _ in candidate_parameters:
                        state.z[name] = (
                            state.z[name] + alpha * old_coupling[name]
                        )
                    for name, _ in candidate_parameters:
                        state.gamma[name] = self.element_prox(
                            state.z[name], kappa
                        )
                    state.mask = _support_masks(
                        state.gamma, support_threshold
                    )
                ...
                # Group Conv path: calculate the next local state first.  If
                # it overshoots, discard it and retain the current feasible
                # state without cloning a large rollback buffer.
                next_theta_delta = {}
                next_z = {}
                next_gamma = {}
                for (name, _), gradient in zip(
                    ...
                    next_gamma[name] = group_lasso_prox(
                        next_z[name], kappa, group_mode
                    )
                next_mask = group_support_masks(
                    next_gamma, group_mode, support_threshold
                )
                support_count = selected_group_count(
                    next_mask, candidate_parameters, group_mode
                )
                if support_count < max_support:
                    state = LBIState(
                        theta_delta=next_theta_delta,
                        gamma=next_gamma,
                        z=next_z,
                        mask=next_mask,
                    )
                    feasible_state = state
                    feasible_step = stage1_steps_completed
                    continue
                if support_count == max_support:
                    state = LBIState(
        ...
        state.mask = (
            group_support_masks(state.gamma, group_mode, support_threshold)
            if group_mode is not None
            else _support_masks(state.gamma, support_threshold)
        )
        support_count = (
            selected_group_count(state.mask, candidate_parameters, group_mode)
            if group_mode is not None
            else _support_count(state.mask)
        )
        support_scalar_count = (
            selected_scalar_count(state.mask)
            if group_mode is not None
            else support_count
        )
        support_scalar_ratio = (
            float(support_scalar_count / candidate_count)
            if candidate_count > 0
            else 0.0
        )
        support_ratio = (
            float(support_count / total_group_count)
            if group_mode is not None and total_group_count > 0
            else support_scalar_ratio
        )
        ...
        if group_mode is None:
            # Preserve the frozen FC writeback implementation byte-for-byte.
            applied_parameters = {
            ...
            # Mathematically the dense interpolation equals base off-mask, but
            # evaluating (1-omega)*base + omega*base in float32 can introduce
            # 1-ULP drift.  Write back only selected Conv coordinates so the
            # strict group support remains exact in the persistent model.
            applied_parameters = {}
            for name, _ in candidate_parameters:
                selected = state.mask[name].to(
                    device=base_parameters[name].device, dtype=torch.bool
                )
                interpolated = (
                    (1.0 - omega) * base_parameters[name]
                    + omega * refined_parameters[name]
                )
                applied_parameters[name] = torch.where(
                    selected, interpolated, base_parameters[name]
                )
        ...
        support_over_model_ratio = (
            float(support_scalar_count / self.total_model_param_count)
            if self.total_model_param_count > 0
            else 0.0
        )
        budget_diagnostics = compute_lbi_step_budget_diagnostics(
            requested_budget=requested_budget,
            candidate_scope_param_count=budget_pool_count,
        ...
        statistics = {
            ...
            **stage2_loss_parts,
            "support_param_count": support_scalar_count,
            "support_over_scope_ratio": support_scalar_ratio,
            "support_over_model_ratio": support_over_model_ratio,
            "selected_param_count": support_scalar_count,
            "selected_over_scope_ratio": support_scalar_ratio,
        ...
        if gradient_accumulator is not None:
            statistics["full_objective_gradient_accumulation"] = True
        if group_mode is not None:
            statistics.update(
                {
                    "group_mode": group_mode,
                    "total_group_count": total_group_count,
                    "max_group_count": max_support,
                    "selected_group_count": support_count,
                    "realized_group_ratio": (
                        float(support_count / total_group_count)
                        if total_group_count > 0
                        else 0.0
                    ),
                    "selected_scalar_count": support_scalar_count,
                    "realized_scalar_ratio": support_scalar_ratio,
                    "stage1_support_scalar_ratio": support_scalar_ratio,
                }
            )
        return LBIResult(
            state=state.clone(),
