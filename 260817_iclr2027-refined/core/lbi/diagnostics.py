...
def _linear_quantile(values, quantile):
    """Deterministic linear quantile for lightweight protocol diagnostics."""
...
def compute_lbi_run_budget_diagnostics(step_records):
    ...
    for index, step in enumerate(steps):
        ...
        try:
            ...
            # Conv Group-LBI budgets groups; the frozen FC fields remain the
            # scalar fallback so existing FC summaries stay byte-for-byte
            # compatible in meaning.
            is_group_budget = step.get("total_group_count") is not None
            group_budget_flags.append(is_group_budget)
            if is_group_budget:
                candidate_count = int(step["total_group_count"])
                if step.get("selected_group_count") is None:
                    return unavailable_lbi_budget_diagnostics(
                        f"missing_selected_group_count_at_{index}"
                    )
                support_count = int(step["selected_group_count"])
            else:
    ...
    if any(group_budget_flags):
        if not all(group_budget_flags):
            return unavailable_lbi_budget_diagnostics(
                "mixed_group_and_scalar_budget_records"
            )
        target_count = annotated[0]["target_support_count"]
        if target_count > 0:
            utilizations = [
                step["stage1_support_count"] / target_count
                for step in annotated
            ]
            below_95_count = sum(value < 0.95 for value in utilizations)
            diagnostics.update(
                {
                    "group_utilization_min": min(utilizations),
                    "group_utilization_mean": (
                        sum(utilizations) / len(utilizations)
                    ),
                    "group_utilization_p05": _linear_quantile(
                        utilizations, 0.05
                    ),
                    "group_utilization_below_95_count": int(
                        below_95_count
                    ),
                    "group_utilization_below_95_fraction": (
                        below_95_count / len(utilizations)
                    ),
                }
            )
        else:
            diagnostics.update(
                {
                    "group_utilization_min": None,
                    "group_utilization_mean": None,
                    "group_utilization_p05": None,
                    "group_utilization_below_95_count": None,
                    "group_utilization_below_95_fraction": None,
                }
            )
    return diagnostics
