...
| DeiT/Transformer TTA backbone | 部分已有 | 已有严格本地 W0 加载、TTDA source-only 直接评测、OTTA source-only 流式评测、OTTA full-dense/candidate-dense 可训练 baseline、OTTA group_random 稀疏 baseline、OTTA group_magnitude 稀疏 baseline 和 OTTA group_saliency 稀疏 baseline；Group Split-LBI structural adaptation 尚未接入 |
| Transformer structural groups | 部分已有 | `shot_otta/adaptation/deit_groups.py` 已实现 paired Q-K/V-O/FFN 的 group registry、随机 group selector、W0 |W|-sum 的 magnitude group selector（`group_magnitude_scores`/`select_magnitude_groups`）、梯度 L2-norm 的 saliency group selector（`group_gradient_saliency_scores`）、通用 Top-K 选择（`select_top_groups`）和 per-parameter mask/scatter；group prox / Group Split-LBI 尚未实现 |
| OTTA | source-only、full-dense、candidate-dense、group_random、group_magnitude、group_saliency 与 legacy FC 已实现 | `evaluate_deit_otta.py` 提供 DeiT 零适配流式 control；`evaluate_deit_otta_full_dense.py --variant {full_dense,candidate_dense}` 提供 SHOT objective + AdamW 的 dense update（full-dense 更新除 classifier/head 外的全部参数即 `all_except_head`，candidate-dense 仅最后 3 block 的 qkv/proj/fc1/fc2 weight，两者的 bias/LN/head 均冻结）；`evaluate_deit_otta_random_group.py` 提供 matched-budget 随机 structural-group 基线（`budget=active_groups/6912`，向上取整，static mask，`strict_masked_delta`，3-mask mean±std）；`evaluate_deit_otta_magnitude.py` 提供 matched-budget magnitude structural-group 基线（W0 上按 group 内 `sum_abs_weight` 选 Top-K，static mask，`strict_masked_delta`，确定性单 mask）；`evaluate_deit_otta_saliency.py` 提供 matched-budget saliency structural-group 基线（每步 backward 后按 group 梯度 L2-norm 选 Top-K，`gradient_l2_norm`，每步重建 mask，`per_step_masked_accumulation`）；legacy ResNet/VGG 路径支持每 batch 适配。DeiT Group Split-LBI OTTA 尚未实现 |
...
├── evaluate_deit_otta_magnitude.py  # DeiT OTTA magnitude structural-group 可训练评测入口
├── evaluate_deit_otta_saliency.py  # DeiT OTTA saliency structural-group 可训练评测入口
...
│   │   └── deit_groups.py        # paired Q-K/V-O/FFN group registry + random/magnitude/saliency selectors
│   ├── otta/                     # DeiT OTTA 协议生命周期（source-only + full-dense + group_random + group_magnitude + group_saliency）
...
├── configs/                     # 单次运行配置（含 deit_otta_{full_dense,candidate_dense,group_random,group_magnitude,group_saliency}.yaml）
├── experiments/                 # 实验矩阵
├── tools/                       # planner、launcher、status、summary（含 run_deit_otta_multi_gpu.py、run_deit_otta_random_group_multi_gpu.py、run_deit_otta_magnitude_multi_gpu.py 与 run_deit_otta_saliency_multi_gpu.py 8 卡调度）
├── scripts/                     # 维护脚本
├── tests/                       # 合成 smoke/工程测试（含 full-dense、random-group、magnitude 与 saliency CPU 契约）
...
3. ~~跑通 TTDA/OTTA Source-only、~~再实现并跑通 Full-dense、last-3 Candidate-dense；（当前完成两个零适配 control、full-dense 与 candidate-dense baseline，均本地实现；服务器仅 AD full-dense 已跑）
