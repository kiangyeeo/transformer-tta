...
sparse runner writes the authoritative ``selection`` record from the start,
so the only job here is to re-aggregate real completed children when a parent
or matrix step failed after the children finished.  No adaptation is rerun.
"""
...
from transformer_come.common import utc_now
...
def recover_completed_run(run_root: Path, *, dry_run: bool = False) -> dict:
    ...
    aggregate = aggregate_matrix(run_root, transfers=TRANSFERS, budgets=FORMAL_BUDGETS)
