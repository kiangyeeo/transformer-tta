"""Sparse non-LBI COME online stream (Random child, Magnitude, Saliency).

Per valid outer batch, protocol section 9: one current COME objective, one
backward, then exactly one strict masked host AdamW step.  The host optimizer
is persistent across batches, there is no scheduler, no Stage-2 and no
omega/EMA writeback.  Off-mask candidate coordinates keep their exact values
and their AdamW moments are cleared.
"""
...
from transformer.group_magnitude.groups import compute_group_l2_scores
...
def run_sparse_transfer(
    ...
    *,
    ...
    try:
        ...
        delta_norms = compute_group_l2_scores(
            [
...
__all__ = ["mask_history_sha256", "named_candidates", "run_sparse_transfer"]
