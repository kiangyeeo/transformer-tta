#!/usr/bin/env bash
# COME-OTTA structural-group Magnitude (static W0 paired L2): 3 budgets x 7 formal transfers = 21 conditions.
...
export TMPDIR=/home/nas3/biod/wangkangyi/tmp/transformer_come_group_magnitude

come_preflight "$VARIANT" "$GPUS" "$CONFIG_PATH" "$OUTPUT_ROOT" "$DRY_RUN" "$RHOS"
cd "$PROJECT_ROOT"

ARGS=(-m transformer_come.group_magnitude matrix
      --config "$CONFIG_PATH"
      --datasets "$DATASETS"
      --rhos "$RHOS"
      --devices "$GPUS"
