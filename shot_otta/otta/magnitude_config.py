"""Configuration resolution for the DeiT OTTA magnitude structural-group
baseline.
...
is ``ceil(budget * total_groups)``.  Selection is deterministic: Top-K groups
by the sum of |W| over each group's 2d scalars, computed from W0 before
adaptation and held fixed for the whole stream.
"""

import os.path as osp

from experiment_identity import resolve_deit_otta_group_magnitude_identity
...
NUM_BLOCKS = 12
VARIANTS = {"group_magnitude": "last_3_block_weights"}
FROZEN_GROUPING = "paired_qk_vo_ffn"
FROZEN_DELTA_SEMANTICS = "strict_masked_delta"
FROZEN_GROUP_SCORE = "sum_abs_weight"
...
def _validate_adaptation(config):
    ...
    if adaptation.get("group_score") != FROZEN_GROUP_SCORE:
        raise ValueError(
            f"adaptation.group_score must be {FROZEN_GROUP_SCORE}"
        )
...
def resolve_config(
    ...
    *,
    ...
    identity = resolve_deit_otta_group_magnitude_identity(
        effective,
