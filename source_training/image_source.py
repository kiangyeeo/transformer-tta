...
def lr_scheduler(optimizer, iter_num, max_iter, gamma=10, power=0.75):
    '''Power Decay 学习率调度，学习率随训练进行逐渐衰减。'''
...
def image_train(resize_size=256, crop_size=224, alexnet=False):
    ...
    return  transforms.Compose([
            ...
            transforms.RandomHorizontalFlip(), # 3.  随机水平翻转（数据增强）
            transforms.ToTensor(),
            normalize
    ])
...
def data_load(args): 
    ...
    # 1.  读取图像列表文件
    txt_src = open(args.s_dset_path). readlines()   # 源域列表
    txt_test = open(args. test_dset_path).readlines() # 测试列表

    # 2. 处理不同的 DA 设置（PDA/ODA 需要过滤类别）
    if not args.da == 'uda':
        # 部分域自适应(PDA)或开放集域自适应(ODA)
        # 需要重新映射标签... 
        label_map_s = {}
    ...
    # 4.  创建 DataLoader
    dsets["source_tr"] = ImageList(tr_txt, transform=image_train()) # 源域训练集
    dset_loaders["source_tr"] = DataLoader(dsets["source_tr"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=False)
    dsets["source_te"] = ImageList(te_txt, transform=image_test()) # 源域验证集
    ...
    dsets["test"] = ImageList(txt_test, transform=image_test()) # 测试集（目标域）
...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            # 原代码: data = iter_test. next()
            data = next(iter_test)  # 新代码: 使用 next() 函数替代 . next() 
            inputs = data[0]
            labels = data[1]
            inputs = inputs.cuda()
...
def cal_acc_oda(loader, netF, netB, netC):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            # 原代码: data = iter_test.next()
            data = next(iter_test)  # 新代码: 使用 next() 函数替代 .next() 方法
            inputs = data[0]
            labels = data[1]
            inputs = inputs.cuda()
            outputs = netC(netB(netF(inputs)))
...
def train_source(args):
    ...
    for k, v in netF.named_parameters():
        param_group += [{'params': v, 'lr': learning_rate*0.1}] # 主干网络用较小学习率
    for k, v in netB.named_parameters():
        param_group += [{'params': v, 'lr': learning_rate}] # 新层用正常学习率
    ...
    # 新代码: 在循环外先初始化 iter_source，避免 UnboundLocalError
    ...
    while iter_num < max_iter:
        ...
        # 新代码: 使用 next() 函数，并用 StopIteration 捕获迭代结束
        try:
        ...
        outputs_source = netC(netB(netF(inputs_source))) # 前向传播
        classifier_loss = CrossEntropyLabelSmooth(num_classes=args.class_num, epsilon=args.smooth)(outputs_source, labels_source)     # 计算损失（Label Smoothing 交叉熵），不是one-hot，防止模型过于自信，提高泛化能力
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data-root', type=str, required=True,
                        help='Directory containing <dataset>/<domain>_list.txt files')
    ...
    folder = osp.abspath(osp.expanduser(args.data_root))
