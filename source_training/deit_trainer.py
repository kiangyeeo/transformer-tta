...
def evaluate(model, loader, criterion, *, device, amp, num_classes):
    model.eval()
    total_loss = 0.0
    count = 0
...
def run_source_training(
    ...
    *,
    ...
    if start_epoch >= training_config["epochs"]:
        raise ValueError(
            f"Resume state has already completed epoch {start_epoch}; "
            f"configured epochs={training_config['epochs']}"
        )
