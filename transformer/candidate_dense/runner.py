"""Run one candidate-dense online SHOT stream followed by independent FO."""
...
def _classifier_hash(model) -> str:
    ...
    return hash_tensors(classifier.state_dict().items())


def _named_candidates(model, names: tuple[str, ...]):
...
def run_transfer(
    ...
    *,
    ...
    model_loader=load_candidate_dense_model,
) -> dict:
    """Run exactly one protocol-aligned candidate-dense transfer."""
    ...
    try:
        ...
        if model.training:
            raise RuntimeError("Candidate-dense adaptation must keep DeiT in eval mode")
        candidate_names = tuple(name for name, _ in trainable)
        ...
        # Model construction consumes RNG; reset so augmentation depends only
        # on the formal stream seed and the fixed DataLoader worker seeds.
        set_reproducibility(config["formal_seed"], config["runtime"]["deterministic"])
        ...
        progress = tqdm(
            online_loader,
            desc=f"candidate-dense {config['dataset']} {config['transfer']}",
            unit="batch",
            dynamic_ncols=True,
            leave=True,
        ...
        candidate_state_after_stream = hash_tensors(
            _named_candidates(model, candidate_names)
        )
        frozen_state_after_stream = hash_tensors(
            frozen_named_state(model, candidate_name_set)
        )
        if candidate_state_after_stream == candidate_state_before:
            raise RuntimeError("Candidate-dense stream did not change candidate weights")
        if frozen_state_after_stream != frozen_state_before:
            raise RuntimeError("A frozen parameter or model buffer changed during adaptation")
        if model_state_after_stream == model_state_before:
            raise RuntimeError("Candidate-dense stream did not change model state")
        if _classifier_hash(model) != classifier_state_before:
        ...
        with torch.inference_mode():
            for batch_index, (images, labels, indices) in enumerate(
                fo_progress, start=1
            ):
                images = images.to(device, non_blocking=True)
                _sync(device)
        ...
        frozen_state_after_fo = hash_tensors(
            frozen_named_state(model, candidate_name_set)
        )
        if model_state_after_fo != model_state_after_stream:
            raise RuntimeError("Read-only FO pass changed final model state")
        if frozen_state_after_fo != frozen_state_before:
            raise RuntimeError("Read-only FO pass changed frozen model state")
        if _classifier_hash(model) != classifier_state_before:
        ...
        summary = {
            ...
            **scope_record,
            ...
            "frozen_state_sha256_after_fo": frozen_state_after_fo,
