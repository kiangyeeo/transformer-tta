"""CLI for single-transfer, dry-run, and GPU-matrix execution."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Protocol-aligned DeiT-S candidate-dense SHOT-OTTA baseline"
    )
...
def _default_transfer_output(raw: dict, dataset: str, source: str, target: str) -> Path:
    ...
    return (
        ...
        / f"single_candidate_dense_seed2026_{stamp}"
        / "results"
        / dataset
        / f"{source}-{target}"
    )
        ...
        tasks.append(
            {
                ...
                "candidate_tensor_count": resolved["adaptation"][
                    "candidate_tensor_count"
                ],
                "candidate_scalar_count": resolved["adaptation"][
