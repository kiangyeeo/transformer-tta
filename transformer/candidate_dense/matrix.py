"""One-process-per-GPU scheduler for candidate-dense transfers."""
...
def new_run_root(output_root: Path) -> Path:
    ...
    path = output_root / f"candidate_dense_seed2026_{run_id}"
...
def run_matrix(
    *,
    ...
    failures: list[dict] = []
    progress = tqdm(
        total=len(transfers), desc="candidate-dense matrix", unit="transfer"
    )
    ...
    if failures:
        ...
        raise RuntimeError(f"Candidate-dense matrix failed: {details}")
