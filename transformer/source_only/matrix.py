"""One-process-per-GPU scheduler for the seven source-only transfers."""
...
from .config import select_transfers


def new_run_root(output_root: Path) -> Path:
    run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%S.%fZ")
    path = output_root / f"source_only_seed2026_{run_id}"
...
def run_matrix(
    *,
    ...
    failures: list[dict] = []
    progress = tqdm(total=len(transfers), desc="source-only matrix", unit="transfer")

    while pending or active:
        while pending and available:
            dataset, source, target = pending.pop(0)
            assigned = available.pop(0)
            task_name = f"{dataset}_{source}-{target}"
            result_dir = run_root / "results" / dataset / f"{source}-{target}"
            log_path = logs_root / f"{task_name}.log"
            log_handle = open(log_path, "w", encoding="utf-8")
            command = [
                sys.executable,
                "-m",
                "transformer.source_only",
                "transfer",
                "--config",
                str(config_path),
                "--dataset",
    ...
    if failures:
        ...
        raise RuntimeError(f"Source-only matrix failed: {details}")
