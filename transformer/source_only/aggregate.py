"""Aggregate transfer summaries without pooling Office samples."""
...
def aggregate_matrix(run_root: Path, transfers=TRANSFERS) -> dict:
    ...
    if office:
        result["office31"] = {
            ...
            "aggregation": "equal-weight mean of transfer accuracies",
    ...
    if visda:
        if len(visda) != 1:
            raise ValueError("Formal VisDA-C aggregation requires one transfer")
        row = visda[0]
        result["visda-c"] = {
