"""One-process-per-GPU scheduler for transfer/budget Magnitude conditions."""
...
def new_run_root(output_root: Path) -> Path:
    ...
    path = output_root / f"group_magnitude_seed2026_{run_id}"
...
def run_matrix(
    *,
    ...
    failures: list[dict] = []
    progress = tqdm(total=len(tasks), desc="group-magnitude matrix", unit="condition")
    ...
    if failures:
        ...
        raise RuntimeError(f"Group-Magnitude matrix failed: {details}")
