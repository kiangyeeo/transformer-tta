"""CLI for one Magnitude condition, dry-run validation, and GPU matrix execution."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Protocol-aligned DeiT-S structural-group Magnitude SHOT-OTTA"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    transfer = subparsers.add_parser(
        "transfer", help="run one transfer and magnitude budget"
    )
...
def _default_transfer_output(raw, dataset, source, target, budget) -> Path:
    ...
    return (
        ...
        / f"single_group_magnitude_seed2026_{stamp}"
        / "results"
        / budget_tag(budget)
        / dataset
        / f"{source}-{target}"
