"""Aggregate Group-Saliency budgets, transfers, and VisDA classes."""
...
def _load_summary(path: Path) -> dict:
    ...
    if summary.get("status") != "completed":
        raise ValueError(f"Saliency condition is not completed: {path}")
    if summary.get("variant") != "group_saliency":
        raise ValueError(f"Condition is not group-saliency: {path}")
    return summary
...
def aggregate_matrix(run_root: Path, *, transfers=TRANSFERS, budgets=FORMAL_BUDGETS) -> dict:
    ...
    for budget in budgets:
        ...
        budget_result = {
            ...
            "active_candidate_scalars_per_step": rows[0][
        ...
        rows.append(
            {
                ...
                "unique_mask_count": summary["selection"]["unique_mask_count"],
                "historical_active_group_union_count": summary["selection"][
                    "historical_active_group_union_count"
                ],
                "mask_history_sha256": summary["selection"]["mask_history_sha256"],
                "summary_path": str(Path(summary["output_dir"]) / "summary.json"),
            }
        )
    if not rows:
        raise ValueError("Cannot write an empty Group-Saliency aggregate")
