"""One-process-per-GPU scheduler for transfer/budget Saliency conditions."""
...
def new_run_root(output_root: Path) -> Path:
    ...
    path = output_root / f"group_saliency_seed2026_{run_id}"
...
def run_matrix(
    *,
    ...
    failures: list[dict] = []
    progress = tqdm(total=len(tasks), desc="group-saliency matrix", unit="condition")
    ...
    if failures:
        ...
        raise RuntimeError(f"Group-Saliency matrix failed: {details}")
