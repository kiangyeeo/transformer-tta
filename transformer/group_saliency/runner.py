"""Run dynamic structural Group-Saliency through online PU and final FO."""
...
from .model import frozen_named_state, hash_tensors, load_group_saliency_model
...
def run_transfer(
    ...
    *,
    ...
    try:
        ...
        if model.training:
            raise RuntimeError("Group-Saliency adaptation must keep DeiT in eval mode")
        ...
        # Model construction must not perturb target permutation/augmentation RNG.
        set_reproducibility(config["formal_seed"], config["runtime"]["deterministic"])
        ...
        selected_kind_totals = {kind: 0 for kind in ("QK", "VO", "FFN")}
        selected_block_totals = {str(block): 0 for block in (9, 10, 11)}
        mask_change_count = 0
        previous_group_ids: tuple[int, ...] | None = None
        backward_calls = 0
        final_masks = None
        ...
        for batch_index, (images, labels, indices) in enumerate(progress, start=1):
            ...
            group_ids = select_saliency_group_ids(
                scores, config["selection"]["requested_budget"]
            )
            ...
            progress.set_postfix(
                ...
                groups=selection_record["realized_group_count"],
        ...
        if model_state_after_stream == model_state_before:
            raise RuntimeError("Group-Saliency stream did not change model state")
        if _classifier_hash(model) != classifier_state_before:
        ...
        with torch.inference_mode():
            for batch_index, (images, labels, indices) in enumerate(fo_progress, start=1):
                ...
                cumulative = (
                    100.0 * fo_meter.confusion.diagonal().sum().item()
        ...
        batch_count = len(mask_hashes)
        selection_summary = {
            **copy.deepcopy(config["selection"]),
            "realized_group_count_per_step": config["selection"]["requested_group_count"],
