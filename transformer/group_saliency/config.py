"""Frozen configuration, budgets, and identities for Group-Saliency SHOT-OTTA."""
...
def _validate_frozen_fields(config: dict[str, Any]) -> None:
    if config.get("schema_version") != 1:
        raise ValueError("group-saliency config schema_version must be 1")
    ...
    if config.get("method") != "shot" or config.get("variant") != "group_saliency":
        raise ValueError("method/variant must be shot/group_saliency")
    expected_selection = {
        ...
        "ranking_source": "current_pre_update_weight_and_current_shot_gradient",
...
def resolve_transfer_config(
    ...
    *,
    ...
    """Resolve one Saliency condition and verify all local assets."""
    ...
    resolved["variant"] = "group_saliency"
    resolved["selection"] = {
        **copy.deepcopy(raw_config["selection"]),
        ...
        "active_candidate_scalars_per_step": budget_group_count(normalized_budget)
        * GROUP_SIZE,
    }
    ...
    resolved["experiment_key"] = (
        f"{dataset}_{source}-{target}_group-saliency_{budget_tag(normalized_budget)}_"
