"""One-process-per-GPU scheduler for the seven full-dense transfers."""
...
def new_run_root(output_root: Path) -> Path:
    ...
    path = output_root / f"full_dense_seed2026_{run_id}"
...
def run_matrix(
    *,
    ...
    failures: list[dict] = []
    progress = tqdm(total=len(transfers), desc="full-dense matrix", unit="transfer")
    ...
    if failures:
        ...
        raise RuntimeError(f"Full-dense matrix failed: {details}")
