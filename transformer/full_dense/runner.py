"""Run one full-dense online SHOT stream followed by independent FO."""
...
from .model import hash_head_state, load_full_dense_model
...
def run_transfer(
    ...
    *,
    ...
    """Run exactly one protocol-aligned full-dense source-to-target condition."""
    ...
    try:
        ...
        model, checkpoint_record, trainable, scope_record = model_loader(config, device)
        if model.training:
            raise RuntimeError("Full-dense adaptation must keep DeiT in eval mode")
        model_state_before = hash_model_state(model)
        head_state_before = hash_head_state(model)
        ...
        # Model construction may consume RNG. Reset so stream augmentation is
        # tied only to the formal seed and loader worker seeds.
        set_reproducibility(config["formal_seed"], config["runtime"]["deterministic"])
        ...
        progress = tqdm(
            online_loader,
            desc=f"full-dense {config['dataset']} {config['transfer']}",
            unit="batch",
            dynamic_ncols=True,
            leave=True,
        ...
        for batch_index, (images, labels, indices) in enumerate(progress, start=1):
            ...
            if hash_head_state(model) != head_state_before:
        ...
        if model_state_after_stream == model_state_before:
            raise RuntimeError("Full-dense online stream did not change model state")
        if hash_head_state(model) != head_state_before:
        ...
        summary = {
            ...
            **scope_record,
            ...
            "frozen_head_sha256_before": head_state_before,
            "frozen_head_sha256_after": hash_head_state(model),
