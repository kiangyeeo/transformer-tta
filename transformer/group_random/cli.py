"""CLI for one Random condition, dry-run validation, and GPU matrix execution."""
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Protocol-aligned DeiT-S structural-group Random SHOT-OTTA"
    )
    subparsers = parser.add_subparsers(dest="command", required=True)
    transfer = subparsers.add_parser(
        "transfer", help="run three masks for one transfer and budget"
    )
...
def _default_transfer_output(raw, dataset, source, target, budget) -> Path:
    ...
    return (
        ...
        / f"single_group_random_seed2026_{stamp}"
        / "results"
        / budget_tag(budget)
        / dataset
        / f"{source}-{target}"
        ...
        tasks.append(
            {
                ...
                "mask_seeds": resolved["selection"]["mask_seeds"],
...
def main(argv=None) -> int:
    ...
    if args.dry_run:
        _dry_run_matrix(
            raw, args.config, args.datasets, budgets, devices, output_root
        )
        return 0
