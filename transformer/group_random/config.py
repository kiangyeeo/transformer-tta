"""Frozen configuration, budgets, and identities for Group-Random SHOT-OTTA."""
...
GROUP_SIZE = 768
NUM_RANDOM_MASKS = 3
MASK_SEEDS = tuple(FORMAL_SEED * 100 + index for index in range(NUM_RANDOM_MASKS))
...
def _validate_frozen_fields(config: dict[str, Any]) -> None:
    if config.get("schema_version") != 1:
        raise ValueError("group-random config schema_version must be 1")
    ...
    if config.get("method") != "shot" or config.get("variant") != "group_random":
        raise ValueError("method/variant must be shot/group_random")
    expected_selection = {
...
def resolve_transfer_config(
    ...
    *,
    ...
    resolved["experiment_key"] = (
        f"{dataset}_{source}-{target}_group-random_{budget_tag(normalized_budget)}_"
