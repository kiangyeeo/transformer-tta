#!/usr/bin/env python3
"""CPU contracts for the DeiT OTTA magnitude structural-group baseline.

Covers the W0 |W|-sum group ranking, deterministic Top-K selection, budget
ceiling, config/identity resolution, and a full end-to-end run with a
...
from shot_otta.adaptation.deit_groups import (  # noqa: E402
    ...
    select_magnitude_groups,
)
from shot_otta.otta.magnitude import (  # noqa: E402
    run_deit_otta_magnitude_experiment,
)
from shot_otta.otta.magnitude_config import resolve_config  # noqa: E402
...
class MagnitudeTinyDeiT(nn.Module):
...
def _base(root):
    config = prepare_assets(
        root, CONFIG_PATH, runs_directory="magnitude_runs"
    )
...
def _state_dict():
    ...
    for block in (9, 10, 11):
        state[f"blocks.{block}.attn.qkv.weight"] = torch.zeros(1152, 384)
        state[f"blocks.{block}.attn.proj.weight"] = torch.zeros(384, 384)
        state[f"blocks.{block}.mlp.fc1.weight"] = torch.zeros(1536, 384)
        state[f"blocks.{block}.mlp.fc2.weight"] = torch.zeros(384, 1536)
    return state


def _check_magnitude_selection():
    state = _state_dict()
    # Q-K group 0: |Q row 0| + |K row 0| = 100.
    state["blocks.9.attn.qkv.weight"][0, 0] = 100.0
    # Q-K group 5: |K row 5| = 50.
    state["blocks.9.attn.qkv.weight"][384 + 5, 1] = 50.0
    # FFN group 10: global index 768 + 10 = 778.
    state["blocks.9.mlp.fc1.weight"][10, 2] = 30.0

    scores = group_magnitude_scores(state)
    assert scores.numel() == TOTAL_GROUP_COUNT == 6912
    assert abs(float(scores[0]) - 100.0) < 1.0e-9
    assert abs(float(scores[5]) - 50.0) < 1.0e-9
    assert abs(float(scores[778]) - 30.0) < 1.0e-9
    assert float(scores.sum()) == 180.0

    selected = select_magnitude_groups(state, 3)
    assert selected == [0, 5, 778]
    # Determinism: repeated selection is identical.
    assert select_magnitude_groups(state, 3) == selected
    # Exact-K and bounds.
    assert len(select_magnitude_groups(state, 35)) == 35
    assert len(set(select_magnitude_groups(state, 35))) == 35
    assert select_magnitude_groups(state, 0) == []
    assert select_magnitude_groups(state, TOTAL_GROUP_COUNT) == list(
...
def _check_config_contract(base):
    ...
    assert effective["variant"] == "group_magnitude"
    ...
    assert effective["adaptation"]["group_score"] == "sum_abs_weight"
    ...
    assert scientific["protocol"] == (
        "sequential_target_stream_group_magnitude_adaptation"
    )
    assert scientific["adaptation"]["grouping"] == "paired_qk_vo_ffn"
    assert scientific["adaptation"]["delta_semantics"] == "strict_masked_delta"
    assert scientific["adaptation"]["selection"] == "magnitude"
    assert scientific["adaptation"]["group_score"] == "sum_abs_weight"
    assert scientific["adaptation"]["active_groups"] == 35
    assert "group_magnitude" in effective["experiment_key"]
    return effective
...
def _check_validation(base):
    ...
    bad = copy.deepcopy(base)
    bad["adaptation"]["group_score"] = "mean_abs_weight"
    _expect_value_error(bad, "group_score")
    bad = copy.deepcopy(base)
...
def _check_end_to_end(root, base):
    ...
    effective = resolve_config(base, PROJECT_ROOT)
    MagnitudeTinyDeiT.total_forward_calls = 0
    summary = run_deit_otta_magnitude_experiment(
        effective, PROJECT_ROOT, model_factory=tiny_factory
    )
    assert summary["status"] == "completed"
    assert summary["variant"] == "group_magnitude"
    ...
    assert len(summary["selected_groups"]) == 35
    assert len(set(summary["selected_groups"])) == 35
    assert len(summary["selected_group_scores"]) == 35
    ...
    assert MagnitudeTinyDeiT.total_forward_calls == 6

    output = Path(summary["output_dir"])
    assert (output / "manifest.json").is_file()
    assert (output / "summary.json").is_file()
    assert (output / "mask.pt").is_file()
    assert (output / "metrics.jsonl").is_file()
    assert summary["delta_nonzero_scalars"] == 35 * SCALARS_PER_GROUP
    metrics = [
        json.loads(line)
...
def main():
    with tempfile.TemporaryDirectory(prefix="deit_otta_magnitude_") as root:
        base = _base(root)
        _check_magnitude_selection()
        _check_budget()
        _check_config_contract(base)
        _check_identity_binds(base)
        _check_validation(base)
        _check_end_to_end(root, base)
    print("DeiT OTTA magnitude structural-group baseline tests passed")
    return 0
