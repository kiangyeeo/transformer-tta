...
def variant_config(variant):
    return {"variant": variant, "requested_budget": 0.1, "selection_seed": 2020, "optimization": {"lr": 0.001, "lr_decay1": 0.1, "lr_decay2": 1.0}, "lbi": {"alpha": 0.1, "kappa": 1.0, "nu": 1.0, "omega": 0.1, "stage1_max_steps": 1, "budget_tolerance": 0.0, "stage2_lr": 0.001, "stage2_steps": 1, "delta_nonzero_tolerance": 0.0}}


def main():
    config = resolve_effective_config(load_yaml(osp.join(PROJECT_DIR, "configs", "shot_otta_visda.yaml")), WORKSPACE_ROOT)
    ...
    order1, meta1 = build_target_order(2020, 20)
    order2, meta2 = build_target_order(2020, 20)
