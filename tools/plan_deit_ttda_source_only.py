...
def _command_args(config_path, effective):
    ...
    return [
        sys.executable,
        osp.join(PROJECT_ROOT, "evaluate_deit_ttda.py"),
        "--config",
...
def build_plan(matrix, base_config, config_path, matrix_path):
    ...
    for dataset in ("office31", "visda-c"):
        for source, target in transfers[dataset]:
            for seed in matrix["seeds"]:
                ...
                raw["seed"] = seed
                effective = resolve_config(raw, PROJECT_ROOT)
                entries.append(
                    {
    ...
    if len(set(keys)) != len(keys) or len(set(hashes)) != len(hashes):
        raise RuntimeError("TTDA source-only plan identities are not unique")
    if len(entries) != 7:
