#!/usr/bin/env python3
"""Dispatch DeiT OTTA magnitude structural-group runs across GPUs.
...
Each job selects its W0-based |W|-sum Top-K groups once and adapts only those
groups, so the per-budget result is a single deterministic ``summary.json``.
"""
...
ENTRYPOINT = PROJECT_ROOT / "evaluate_deit_otta_magnitude.py"
...
def config_output_root():
    config_path = PROJECT_ROOT / "configs" / "deit_otta_group_magnitude.yaml"
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Run DeiT OTTA magnitude group baseline across GPUs."
    )
    parser.add_argument(
        "--budgets",
