...
def _selection_seed_mode(row, plan_by_identity):
    ...
    if row.get("variant") != "module_random":
        return None
...
def _aggregate_lbi_budget_diagnostics(group_rows):
    if not group_rows or group_rows[0].get("variant") != "module_lbi":
        return {}
    available_rows = [
        row
        for row in group_rows
...
def _combine_lbi_transfer_diagnostics(transfer_rows):
    if not transfer_rows or transfer_rows[0].get("variant") != "module_lbi":
        return {}
